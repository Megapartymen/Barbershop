...
void geom(triangle hullGeomInput IN[3], inout TriangleStream<fragInput> tristream, uint InstanceID : SV_GSInstanceID)
{
	...
	uint startingLayer = InstanceID + GEOM_START_SHELL;
	if (startingLayer > (uint)visibleLayers) return;
	...
	float movementRange = IN[0].WORLDTHICKNESS * (0.10 + maxMaxZFur + (_CameraProximityTouch * 0.5));
	float faceDistance = max((IN[0].VIEWDISTANCE + IN[1].VIEWDISTANCE + IN[2].VIEWDISTANCE) * 0.333333333333, movementRange * 0.5);
	...
	for (float layer = startingLayer; layer <= visibleLayers; layer += GEOM_INSTANCES)
	{
