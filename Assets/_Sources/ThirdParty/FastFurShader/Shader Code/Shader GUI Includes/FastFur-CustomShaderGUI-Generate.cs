...
namespace WarrensFastFur
{
	...
	public class GenerateFunctions
	{
		...
		private void drawHairs(Color[] pixelsA, Color[] pixelsB, int texWidth, int texHeight, float minSize, float maxSize, float count, float taper, float minHeight, float maxHeight, float minShift, float maxShift, float minHighlight, float maxHighlight)
		{
			int actual = 0;
			for (int i = 0; i < count; i++)
			{
				for (int j = 0; j < 250; j++)
				{
					...
					if (checkHair(pixelsA, texWidth, texHeight, size, x, y))
					{
		...
		private bool checkHair(Color[] pixels, int texWidth, int texHeight, float size, float x, float y)
		{
			size = (size + 1) * 0.5f;
			...
			for (int x2 = -intSize; x2 <= intSize; x2++)
			{
				for (int y2 = -intSize; y2 <= intSize; y2++)
				{
					...
					if (pixels[(int)x3 + (int)y3 * texWidth].b > 0) return false;
				}
			}
			return true;
		}
