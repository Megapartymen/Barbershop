...
float3 calculateWind(float3 worldPos, float2 uv)
{
	// Start by calculating the wind vector
	float windYCos = cos(radians(_WindAngle));
	float3 windVector = normalize(float3(sin(radians(_WindDirection)) * windYCos, -sin(radians(_WindAngle)), cos(radians(_WindAngle)) * windYCos)) * _WindSpeed;
	...
	float windPosition = dot(worldPos, _WindDirectionActual.xyz);
	...
	// Apply the turbulence noise
	windVector += (waveTotal.xyz * (WIND_TURBULENCE_SCALING_BIAS + (_WindTurbulenceStrength * _WindSpeed)) * WIND_TURBULENCE_SCALING);
