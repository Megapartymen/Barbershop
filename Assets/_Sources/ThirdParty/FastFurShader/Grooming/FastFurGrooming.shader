		...
		SubShader
		{
			...
			Pass
			{
				...
				void geom(triangle v2f IN[3], inout TriangleStream<v2f> tristream)
				{
					...
					relativeDensity[0] = distance(IN[0].pos, IN[1].pos) / distance(IN[0].worldPos, IN[1].worldPos);
					relativeDensity[1] = distance(IN[0].pos, IN[2].pos) / distance(IN[0].worldPos, IN[2].worldPos);
					relativeDensity[2] = distance(IN[1].pos, IN[2].pos) / distance(IN[1].worldPos, IN[2].worldPos);
				...
				float4 frag(v2f i) : SV_Target
				{
					...
					float dist = abs(tex2D(_DirectionTex, i.uv).a);
					...
					float2 newEditVector = (targetVector * fade) + (preEditVector * (1 - fade));

					// We want to move the hairs closer to the target, but we also want to 
					// move them further away from their preEdit starting position.
					// That way hairs that pass through the centre of the brush won't decrease the
					// combing once the centre passes by and they are on the edge of the brush.
					float2 combVector;
					if(_FurCombingMode == 2) // Direction only
					{
						// If we are only combing the direction, then move towards it
						newEditVector = normalize(newEditVector) * targetCombStrength;
						float dotCurrentEdit = dot(normalize(currentEditVector), targetCombDirection);
						float dotNewEdit = dot(normalize(newEditVector), targetCombDirection);
						combVector = dotNewEdit >= dotCurrentEdit ? newEditVector : currentEditVector;
					}
					else
					{
						// Otherwise we are combing the strength, so which is further away from the pre-edit direction?
						float currentEditOffset = abs(distance(currentEditVector, preEditVector));
						float newEditOffset = abs(distance(newEditVector, preEditVector));
						combVector = newEditOffset >= currentEditOffset ? newEditVector : currentEditVector;
					}
					...
					float targetDensity = _FurBrushMode == 3 ? maskCol.a : _FurGroomFurDensity;
					// Relative density
					...
					float4 finalCol = dist < _FurGroomBrushRadius ? float4(combVector * 0.5 + 0.5, furLength, targetDensity) : col;
					...
					finalCol.a = _FurGroomFurDensitySetAll > 0 ? (_FurBrushMode == 3 ? maskCol.a : _FurGroomFurDensity) : finalCol.a;
					return(finalCol);
				}
